...
## 3. Semantic Enrichment with `cie_lookup()`

After normalization, standardized clinical descriptions are assigned to facilitate result interpretation. `cie_lookup()` performs this task through an optimized query to the integrated catalog, returning full metadata for each code.

```{r lookup}
# Extracting descriptions for unique codes
unique_codes <- unique(discharges$DIAG1_NORM)

diagnosis_dictionary <- cie_lookup(
  code = unique_codes,
  full_description = TRUE
)

# Integrating descriptions into the main dataframe
discharges_full <- discharges %>%
  left_join(diagnosis_dictionary, by = c("DIAG1_NORM" = "codigo"))

head(discharges_full %>% select(DISCHARGE_ID, DIAG1, descripcion))
```
