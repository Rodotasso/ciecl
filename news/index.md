...
ahora valida `category` y `cie_table(NA)` aborta como input inválido en
lugar de reportar “código no encontrado: NA”.

- **Validaciones endurecidas** (clase `ciecl_invalid_input`):
  [`cie_short()`](https://rodotasso.github.io/ciecl/reference/cie_short.md)
  valida que `category` sea NULL o un string escalar no-NA (antes un
  vector o numérico derivaba en un error duro no tipado);
  [`cie_table()`](https://rodotasso.github.io/ciecl/reference/cie_table.md)
  rechaza `code = NA` como input inválido (antes caía en “Código no
  encontrado: NA”).
- **Orden determinista en
  [`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md)**:
  el modo vector exacto (`IN (...)`) ahora incluye `ORDER BY codigo`,
  igual que los demás caminos de la función.
...
- **Limpieza de metadatos**: se completa la eliminación del
  ...
  `writexl` sale de `Suggests` (sin uso ejecutable en el paquete) y de
  las tablas de instalación de las viñetas; se verificó que
  `Config/roxygen2/version` en DESCRIPTION es el mecanismo estándar del
  roxygen2 vigente (reemplaza a `RoxygenNote`; `document()` la
  re-escribe automáticamente) y se mantiene con el valor correcto;
  `codemeta.json` registra `dateModified`.
- **README y viñetas bilingües**: los chunks de instalación y
  comorbilidad del README ahora respetan la condicional de idioma (el
  README en inglés ya no muestra comentarios en español); el encabezado
  final de la viñeta en inglés se alinea al registro de la española
  (“Collaboration and Support”).
- **Tests**: nuevas aserciones para el warning de rango invertido en
  [`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md),
  los errores tipados de `threshold`/`max_results` en
  [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md),
  la coerción de `default = NA_real_`/`NaN` en
  [`cie_describe()`](https://rodotasso.github.io/ciecl/reference/cie_describe.md)
  y el NULL del camino sin match de `sigla_to_codigo()`; literales
  non-ASCII de `test-encoding.R` convertidos a escapes `\uXXXX`.

*Input validation hardened (class `ciecl_invalid_input`):
[`cie_short()`](https://rodotasso.github.io/ciecl/reference/cie_short.md)
requires `category` to be NULL or a non-NA length-1 string, and
`cie_table(NA)` now aborts as invalid input instead of reporting “code
not found: NA”. Exact vector mode in
[`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md)
now sorts by `codigo` for deterministic output. Metadata cleanup: the
unused `@importFrom dplyr select` removal is completed (all uses were
already qualified
[`dplyr::select()`](https://dplyr.tidyverse.org/reference/select.html)),
`writexl` leaves `Suggests` (no executable use in the package),
`Config/roxygen2/version` in DESCRIPTION was verified to be the standard
mechanism of the current roxygen2 (it replaces `RoxygenNote` and is
rewritten automatically by `document()`) and is kept with its correct
value, and `codemeta.json` records `dateModified`. README install and
comorbidity chunks now follow the language conditional, and the English
vignette’s closing heading matches the Spanish register (“Collaboration
and Support”). Tests added for the inverted-range warning, typed
`threshold`/`max_results` errors, `default` coercion in
[`cie_describe()`](https://rodotasso.github.io/ciecl/reference/cie_describe.md),
and the NULL no-match path of `sigla_to_codigo()`.*

### Correcciones menores — nits de auditoría (2026-09-15)

Sin cambios en la API pública; un cambio de comportamiento acotado:
[`cie_table()`](https://rodotasso.github.io/ciecl/reference/cie_table.md)
ahora requiere un único código; pasar un vector de largo \>1 (uso nunca
documentado) aborta con error claro en lugar del error de `gt`.

- **Bug fix —
  [`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md)**:
  una consulta `SELECT` legítima que comenzaba con un comentario (`--` o
  `/* */`) abortaba como “no SELECT”; la validación ahora corre después
  de remover comentarios y strings.
- **Validaciones endurecidas** (clase `ciecl_invalid_input`):
  [`cie_table()`](https://rodotasso.github.io/ciecl/reference/cie_table.md)
  valida que `code` sea un string de longitud 1;
  [`cie_describe()`](https://rodotasso.github.io/ciecl/reference/cie_describe.md)
  valida que `default` sea character escalar o `NA`.

*Bug fix:
[`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md)
no longer rejects a legitimate `SELECT` that starts with a comment (`--`
or `/* */`); the check now runs after stripping comments and string
literals. Input validation hardened (class `ciecl_invalid_input`):
[`cie_table()`](https://rodotasso.github.io/ciecl/reference/cie_table.md)
requires `code` to be a length-1 string (a behavior change: a length \>1
vector, never a documented use, now aborts with a clear error instead of
the `gt` error) and
[`cie_describe()`](https://rodotasso.github.io/ciecl/reference/cie_describe.md)
requires `default` to be a scalar character or `NA`.*

### Correcciones de robustez — auditoría interna (2026-09-13)

Primera tanda (Fase A) de la auditoría interna de `R/`. Sin cambios en
la API pública; un cambio de comportamiento acotado: `cie_expand(NA)`
ahora aborta con error claro en lugar de retornar `character(0)`
silenciosamente.

- **Bug fix — rangos en
  [`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md)**:
  un rango como `"E10-E14"` excluía silenciosamente las subcategorías
  del límite superior (`E14.0`–`E14.9`), porque `BETWEEN` con collation
  BINARY compara lexicográficamente (`'E14.9' > 'E14'`). La consulta
  ahora cubre el prefijo del límite superior, por lo que el rango
  incluye todas las subcategorías.
- **Validaciones endurecidas** (errores claros con clase
  `ciecl_invalid_input` en lugar de errores base de R):
  [`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md)
  aborta si `extract = TRUE` recibe más de un código (el contrato
  documentado es escalar);
  [`cie_expand()`](https://rodotasso.github.io/ciecl/reference/cie_expand.md)
  valida que `code` sea un string escalar;
  [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
  valida tipo y ausencia de `NA` en `threshold` y `max_results`;
  [`cie11_search()`](https://rodotasso.github.io/ciecl/reference/cie11_search.md)
  valida `NA` en `max_results`.
- **[`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
  — esquema de salida estable**: la columna `uso_cl` ahora está presente
  en todos los caminos internos (FTS y fallbacks), y
  ...
  resultados vigentes).
- **[`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md)
  vectorial**: los códigos con caracteres inválidos o no encontrados se
  informan en un mensaje agregado (como ya hacía el modo escalar), en
  lugar de descartarse en silencio.
- **[`cie_guide()`](https://rodotasso.github.io/ciecl/reference/cie_guide.md)**:
  la tabla de orientación recomendaba el argumento deprecado
  `expandir = TRUE`; ahora indica el vigente `expand = TRUE`.
...
- **Hardening de
  [`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md)**:
  el escaneo de palabras clave bloqueadas ahora se aplica sobre la
  consulta ya limpia de literales de texto y comentarios, eliminando
  falsos positivos en `SELECT` legítimas (p. ej. `LIKE '%drop%'`); las
  protecciones vigentes no cambian.
- **`CIECL_CACHE_DIR=""`**: una variable de entorno definida pero vacía
  ahora se trata como no definida (fallback a
  [`tools::R_user_dir()`](https://rdrr.io/r/tools/userdir.html)); antes
  derivaba en escribir la caché en el directorio de trabajo.
- **[`cie_norm()`](https://rodotasso.github.io/ciecl/reference/cie_norm.md)**:
  `"E11.X"` (convención “no especificada”) ya no queda como `"E11."`
  malformado; al remover la `X` final se remueve también el punto previo
  si lo hay.
- **Caché SQLite**: la construcción y eliminación de la caché ahora
  chequean el retorno de
  [`file.rename()`](https://rdrr.io/r/base/files.html)/[`file.remove()`](https://rdrr.io/r/base/files.html)
  y advierten con un mensaje informativo si el sistema operativo rechaza
  la operación (típico lock de archivo en Windows), en lugar de
  continuar en silencio.
- **[`cie_comorbid()`](https://rodotasso.github.io/ciecl/reference/cie_comorbid.md)**:
  la documentación del valor de retorno ahora indica correctamente
  `tibble` (antes decía `data.frame`), y `data`, `id` y `code` se
  validan al inicio con errores tipados `ciecl_invalid_input`.
- **[`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
  con texto de solo símbolos** (p. ej. `"!!"`): el camino sin candidatos
  para la búsqueda difusa ahora retorna de inmediato un tibble vacío con
  el esquema correcto; antes calculaba `mean(numeric(0))` y propagaba un
  `NaN` silencioso en los scores.
- **Limpieza interna**: eliminados `@importFrom` sin uso real (`pull`,
  `rowwise`, `tribble`, `matches`, `select`); `NAMESPACE` regenerado.

*Bug fix: code ranges in
[`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md)
(e.g. `"E10-E14"`) now include the upper bound’s subcategories
(`E14.x`), previously dropped silently by the lexicographic `BETWEEN`.
Input validation hardened (`extract = TRUE` scalar-only, scalar
[`cie_expand()`](https://rodotasso.github.io/ciecl/reference/cie_expand.md)
— with `NA` now aborting instead of silently returning `character(0)` —,
typed `threshold`/`max_results` with explicit `NA` guards in
[`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
and
[`cie11_search()`](https://rodotasso.github.io/ciecl/reference/cie11_search.md)).
[`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
output schema is now stable (`uso_cl` present on every internal path)
and `only_uso_cl = TRUE` filters legacy codes before the `max_results`
cap. Vector-mode
[`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md)
reports invalid/not-found codes in a single aggregate message, and
[`cie_guide()`](https://rodotasso.github.io/ciecl/reference/cie_guide.md)
now points to the current `expand` argument instead of the deprecated
`expandir`.*

*Phase B (same audit, low-risk items):
[`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md)
keyword scanning now ignores string literals and comments (no more false
positives on legitimate `SELECT`s, same protections); an empty
`CIECL_CACHE_DIR` falls back to
[`tools::R_user_dir()`](https://rdrr.io/r/tools/userdir.html) instead of
writing to the working directory; `cie_norm("E11.X")` no longer leaves a
dangling dot; cache build/clear now warn if
[`file.rename()`](https://rdrr.io/r/base/files.html)/[`file.remove()`](https://rdrr.io/r/base/files.html)
fail;
[`cie_comorbid()`](https://rodotasso.github.io/ciecl/reference/cie_comorbid.md)
docs correctly state the tibble return and `data`/`id`/`code` are
validated up front;
[`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
with a symbols-only text returns an empty tibble with the correct schema
instead of a silent `NaN`; unused `@importFrom` entries removed.*
...
[@maelle](https://github.com/maelle) en el issue rOpenSci
[\#765](https://github.com/Rodotasso/ciecl/issues/765). Sin cambios en
la API pública.

- **Guía de inicio mejorada**: la frase sobre
  [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) quedó
  en lenguaje simple; el cruce entre la búsqueda difusa y los códigos
  presentes en la base ahora se muestra con código ejecutable
  ([`intersect()`](https://generics.r-lib.org/reference/setops.html)),
  explicitando de dónde salen `E11.9` y `E14.9`; la guía cierra el caso
  de uso filtrando los egresos y resumiéndolos por tipo de diabetes. Los
  mismos cambios se aplicaron a la gemela en inglés
  `case-study-discharges.Rmd`, que queda sincronizada con `ciecl.Rmd`.
- **Caché SQLite**: la ayuda de
  [`cie10_clear_cache()`](https://rodotasso.github.io/ciecl/reference/cie10_clear_cache.md)
  ahora indica que la reconstrucción tras una actualización del paquete
  es automática (la caché guarda la versión del paquete en la tabla
  `cie10_meta` y se reconstruye sola en el primer uso si difiere); los
  casos de uso manual que quedan son caché corrupta o liberación de
  disco. La ayuda de
  [`cie10_disconnect()`](https://rodotasso.github.io/ciecl/reference/cie10_disconnect.md)
  aclara que la desconexión manual solo es necesaria al borrar el `.db`
  por fuera del paquete o al cerrar procesos batch largos. Tres tests
  nuevos cubren el ciclo de reconstrucción por versión.
- **[`cie11_search()`](https://rodotasso.github.io/ciecl/reference/cie11_search.md)**:
  la condición del ejemplo con `vcr` se movió a `@examplesIf`, de modo
  que ya no aparece en la documentación visible.
...
[\#765](https://github.com/Rodotasso/ciecl/issues/765), a partir de los
comentarios de [@maelle](https://github.com/maelle). Cierra todos los
ítems pendientes de la revisión.

- **[`cie11_search()`](https://rodotasso.github.io/ciecl/reference/cie11_search.md)**:
  nuevo argumento `api_key = get_icd_api_key()` con la nueva función
  exportada
  [`get_icd_api_key()`](https://rodotasso.github.io/ciecl/reference/get_icd_api_key.md),
  que centraliza la lectura de las credenciales de la API OMS siguiendo
  el patrón de `httr2`. La ayuda gana una subsección “Seguridad de la
  API key” y las viñetas de instalación (ES/EN) agregan la nota
  correspondiente (ms17).
- **Ejemplos offline con `vcr`**: los ejemplos de
  [`cie11_search()`](https://rodotasso.github.io/ciecl/reference/cie11_search.md)
  ahora corren sin conexión usando cassettes de `vcr` (\>= 2.0.0,
  agregado a `Suggests`); cassette en `inst/_vcr/cie11_search.yml` y
  `tests/testthat/setup-vcr.R` con `filter_sensitive_data` (ms19).
- **Documentación bilingüe**: español primero en CONTRIBUTING y README;
  enlaces y orden de bullets corregidos; tildes en el índice de
  referencia del sitio pkgdown y en mensajes de `cli` (ms01–ms08).
- **Viñetas**: instalación solo con `pak`; nueva guía de inicio con caso
  de uso real que reemplaza a `caso-uso-egresos.Rmd`; nueva viñeta en
  inglés `vignettes/ciecl-en.Rmd` (ms10–ms13).
- **Sitio pkgdown**: footer de `_pkgdown.yml` corregido según el PR
  [\#15](https://github.com/Rodotasso/ciecl/issues/15) de la revisora
  (`developed_by` escalar) (ms09).
- **CONTRIBUTING.md**: nueva sección “Declaración de asistencia con IA”
  (español e inglés) (ms14).
- **Mejoras de código interno**: uso de
  [`anyNA()`](https://rdrr.io/r/base/NA.html),
  [`nzchar()`](https://rdrr.io/r/base/nchar.html) y variables
  explicativas en las funciones revisadas (ms15, ms16, ms18).
- **Tests**: aserciones más específicas (`expect_named()`,
  `expect_length()`, `expect_type()`, `expect_all_false()`) y
  eliminación de `:::` en la suite de tests (ms20).
...
[@yabellini](https://github.com/yabellini) en el issue rOpenSci
[\#765](https://github.com/Rodotasso/ciecl/issues/765). Sin cambios en
la API pública: no se agregan ni eliminan funciones exportadas, no
cambian firmas ni se rompe compatibilidad.

- **Bug de documentación (`cie10_cl`)**: la columna `uso_cl` estaba
  documentada como lógica, pero en realidad es `character` con las
  categorías `"principal"`, `"legado"`, `"etiologico"`,
  `"causa_externa"` y `"causa_externa | principal"`. Corregido el
  `@format` y el ejemplo roto `subset(cie10_cl, uso_cl == TRUE)`, que
  devolvía un conjunto vacío (ybs08, ybs09).
- **`cie10_empty_tibble()`**: se agrega la columna `uso_cl` al esquema
  del tibble vacío interno, evitando inconsistencias al concatenar
  resultados (ybs36).
- **Validación de parámetros obligatorios**:
  ...
  y
  [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
  ahora emiten un error en castellano vía
  [`cli::cli_abort()`](https://cli.r-lib.org/reference/cli_abort.html)
  cuando se las llama sin su argumento obligatorio, en lugar del mensaje
  en inglés de `rlang`/R base (ybs19, ybs22, ybs24, ybs27, ybs30).
- **[`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md)**:
  validación explícita de `query` (tipo character, longitud 1, no NA),
  captura de errores de
  [`DBI::dbGetQuery()`](https://dbi.r-dbi.org/reference/dbGetQuery.html)
  con [`tryCatch()`](https://rdrr.io/r/base/conditions.html) relanzados
  como clase `ciecl_sql_error`, descripción y documentación de `query`
  ampliadas (`FROM`, `ORDER BY`, `GROUP BY`, `HAVING`) y
  `@seealso [cie10_cl]` (ybs12–ybs17). Terminología de los mensajes
  homogeneizada al castellano (consulta/palabra clave/sentencias)
  (ybs14).
- **Ortografía de la ayuda**: se corrigen las tildes faltantes en la
  documentación roxygen de
  [`cie_norm()`](https://rodotasso.github.io/ciecl/reference/cie_norm.md),
  ...
  —deprecated pero exportada— corresponden a ybs25, y las de
  [`cie_short()`](https://rodotasso.github.io/ciecl/reference/cie_short.md)
  y
  [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md)
  a ybs26.
- **[`cie10_clear_cache()`](https://rodotasso.github.io/ciecl/reference/cie10_clear_cache.md)
  /
  [`cie10_disconnect()`](https://rodotasso.github.io/ciecl/reference/cie10_disconnect.md)**:
  ayuda ampliada explicando qué es la caché SQLite, cuándo conviene
  forzar la reconstrucción, qué es el bloqueo del archivo `.db`, por qué
  ocurre y qué pasa si no se libera (ybs10, ybs11).
- **Mensajes en castellano consistente**: en
  [`cie_short()`](https://rodotasso.github.io/ciecl/reference/cie_short.md)
  y
  [`cie_validate_vector()`](https://rodotasso.github.io/ciecl/reference/cie_validate_vector.md)
  el listado de valores usa el conector `" y "` en lugar del `" and "`
  que `cli` aplica por defecto al colapsar vectores; el mensaje de
  [`cie_validate_vector()`](https://rodotasso.github.io/ciecl/reference/cie_validate_vector.md)
  reemplaza `"DB"` por `"base de datos"` y agrega la tilde de
  `"Códigos"` mediante escape unicode, según lo sugerido en la revisión
  (ybs14, ybs20, ybs27).
- **[`cie_map_comorbid()`](https://rodotasso.github.io/ciecl/reference/cie_map_comorbid.md)**:
  ahora emite un
  [`cli::cli_warn()`](https://cli.r-lib.org/reference/cli_abort.html)
  cuando alguna entrada no tiene formato CIE-10 válido (p. ej. `"hola"`
  o `35`), indicando cuántas se clasificaron como `"Otra"`. El
  comportamiento de clasificación no cambia: los códigos CIE-10 válidos
  no mapeados siguen en `"Otra"` (ybs34).
- **[`cie_guide()`](https://rodotasso.github.io/ciecl/reference/cie_guide.md)**:
  se mantiene como función, se agrega un enlace cruzado desde el README
  hacia ella y `@seealso` a ella desde las ayudas de las funciones de
  búsqueda (ybs31).
- **Tests**: nuevos casos de validación para
  [`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md)
  y refuerzo de las aserciones de las dos ramas “sin palabras válidas”
  de
  [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md),
  que ahora verifican que la consulta de respaldo devuelve datos reales,
  no solo que no falla (ybs17, ybs28).
...
- **Getting-started guide improved**: plain-language
  [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html)
  wording, an executable
  [`intersect()`](https://generics.r-lib.org/reference/setops.html) step
  showing where `E11.9`/`E14.9` come from, and a closing section that
  filters and summarizes discharges by diabetes type. The English twin
  `case-study-discharges.Rmd` is now in sync with `ciecl.Rmd`.
- **SQLite cache**:
  [`cie10_clear_cache()`](https://rodotasso.github.io/ciecl/reference/cie10_clear_cache.md)
  docs now state that rebuilding after a package update is automatic
  (cache stores the package version in `cie10_meta` and rebuilds on
  first use when it differs);
  [`cie10_disconnect()`](https://rodotasso.github.io/ciecl/reference/cie10_disconnect.md)
  docs clarify when manual disconnection is actually needed. Three new
  tests cover the version-based rebuild cycle.
- **[`cie11_search()`](https://rodotasso.github.io/ciecl/reference/cie11_search.md)**:
  the `vcr` example guard moved to `@examplesIf`, so it no longer shows
  in the visible documentation.
- **Installation vignettes (ES/EN)**: opening paragraph explaining who
  the guide is for and who can skip it.
