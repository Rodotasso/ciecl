...
- Ejemplos de
  ...
  envueltos en `\donttest{}` para reducir tiempo de check
...
- **FTS5 tabla faltante**: Agregar verificacion independiente de
  `cie10_fts`
  - Si la BD tiene `cie10` pero no `cie10_fts` (cache corrupto), ahora
    se recrea
  - Soluciona error “no such table: cie10_fts” en examples/tests
  - Afecta `get_cie10_db()` en `R/cie-sql.R`
- **Link invalido README**: Cambiar link relativo `DEPENDENCIAS.md` a
  URL absoluta
...
- **Script audit removido de tests/**: Mover `audit_elite_cran.R` a
  `tools/`
...
- **Audit script moved**: `audit_elite_cran.R` moved from tests/ to
  tools/
...
- `test-robustness.R`: +40 nuevos tests
  ...
  - [`cie10_clear_cache()`](https://rodotasso.github.io/ciecl/reference/cie10_clear_cache.md):
    Eliminacion idempotente
  - [`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md):
    Bloqueo de keywords peligrosos (DROP, DELETE, UPDATE, etc.)
...
- Conexiones DB con [`on.exit()`](https://rdrr.io/r/base/on.exit.html)
  garantizado
...
New test files for: - Data generation functions (parsear_cie10_minsal,
generar_cie10_cl) - Dataset integrity validation - Internal functions -
Comorbidity validation - API mocking and real API tests - Expanded
robustness tests - SQL security tests - Edge cases coverage
...
- `cie_comorbid(map="elixhauser")`: Calculo correcto de categorias
  booleanas
...
- `cie_lookup_single()`: Corregir emisión de warning para rangos
  invertidos
  - Usar [`paste0()`](https://rdrr.io/r/base/paste.html) en lugar de
    concatenación con comas en
...
- [`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md):
  Documentación mejorada del parámetro `extract`:
...
- `extract_cie_from_text()`: Función interna para extraer código CIE-10
  de texto con ruido
...
- `test-cie-search.R`: Modificar test de rangos invertidos para usar
  [`suppressWarnings()`](https://rdrr.io/r/base/warning.html)
...
and
[`cie_map_comorbid()`](https://rodotasso.github.io/ciecl/reference/cie_map_comorbid.md)
with synthetic datasets: \* 500-patient Charlson test base with 2007
diagnoses \* 500-patient Elixhauser test base with 3219 diagnoses \*
...
- [`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md):
  Improved documentation for `extract` parameter
- `extract_cie_from_text()`: Internal function to extract ICD-10 codes
  from noisy text
...
- [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md):
  Soporte para **88 siglas medicas** comunes en Chile:
...
- [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md):
  Tolerancia a tildes mejorada:
  - “neumonia” ahora encuentra “neumonía”
  - “rinon” encuentra “riñón”
  - “corazon” encuentra “corazón”
- [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md):
  Busqueda por subcadena como estrategia principal:
  - Mas rapido y preciso para terminos exactos
  - Fuzzy search como fallback para typos
- [`cie_siglas()`](https://rodotasso.github.io/ciecl/reference/cie_siglas.md):
  Nueva funcion para listar todas las siglas medicas soportadas

### Datos

- Dataset `cie10_cl`: Agregados 4 codigos COVID-19 de actualizaciones
  OMS 2021:
...
- [`cie_normalizar()`](https://rodotasso.github.io/ciecl/reference/cie_normalizar.md):
  Manejo robusto de caracteres especiales comunes en datos clinicos:
...
- [`cie_normalizar()`](https://rodotasso.github.io/ciecl/reference/cie_normalizar.md):
  Ahora elimina automaticamente el sufijo “X” de codigos CIE-10 (ej.
  I10X -\> I10, J00X -\> J00). Esto permite trabajar con codigos que
  usan “X” para indicar ausencia de subcategoria adicional.

- [`cie_normalizar()`](https://rodotasso.github.io/ciecl/reference/cie_normalizar.md):
  Preserva X en codigos largos (\>5 chars) donde es placeholder
  obligatorio del 7o caracter de extension en trauma/lesiones (ej.
  S72X01A). Esto asegura compatibilidad con codificacion ICD-10-CM.

## ciecl 0.1.0 (BETA)

> **NOTA**: Esta es una version beta en desarrollo activo. La API puede
> cambiar antes de la version estable 1.0.0.
...
- [`cie_lookup()`](https://rodotasso.github.io/ciecl/reference/cie_lookup.md) -
  Busqueda exacta por codigo (vectorizada)
- [`cie_search()`](https://rodotasso.github.io/ciecl/reference/cie_search.md) -
  Busqueda fuzzy de terminos medicos
- [`cie10_sql()`](https://rodotasso.github.io/ciecl/reference/cie10_sql.md) -
  Consultas SQL directas sobre CIE-10
- [`cie11_search()`](https://rodotasso.github.io/ciecl/reference/cie11_search.md) -
  Busqueda en API CIE-11 OMS (requiere credenciales)

#### Utilidades

- [`cie_expand()`](https://rodotasso.github.io/ciecl/reference/cie_expand.md) -
  Expansion jerarquica de categorias
- [`cie_validate_vector()`](https://rodotasso.github.io/ciecl/reference/cie_validate_vector.md) -
  Validacion formato de codigos
- [`cie_normalizar()`](https://rodotasso.github.io/ciecl/reference/cie_normalizar.md) -
...
- [`cie_comorbid()`](https://rodotasso.github.io/ciecl/reference/cie_comorbid.md) -
  Calculo de indices de comorbilidad
- [`cie_map_comorbid()`](https://rodotasso.github.io/ciecl/reference/cie_map_comorbid.md) -
  Mapeo de categorias de comorbilidad

#### Visualizacion (requiere paquete gt)

- [`cie_table()`](https://rodotasso.github.io/ciecl/reference/cie_table.md) -
  Tablas interactivas GT

#### Generacion de datos

- `generar_cie10_cl()` - Generar dataset desde XLS/XLSX MINSAL (requiere
  readxl)
...
Fuente de datos: [Centro FIC Chile
